...
func setUserAgent(client *autorest.Client) {
	client.UserAgent = fmt.Sprintf("Ernest")
}
...
// Provider : Get an Azure client based on encrypted data
func Provider(subscriptionID, clientID, clientSecret, tenantID, env, cryptoKey string) (*ArmClient, error) {
	var err error
	...
	if cryptoKey != "" {
		...
		if env, err = crypto.Decrypt(env, cryptoKey); err != nil {
			log.Println(err.Error())
			return nil, err
		}
	}
